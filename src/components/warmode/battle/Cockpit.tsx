...
import type { BattleRound, Circuit, DriverId, PointsMapping, RawRace, RawSeason, RoundPowerup } from "@/lib/types";
...
interface CockpitProps {
  ...
  roundPowerups: RoundPowerup[];
  onChanged: () => void;
}

/**
 * The real gameplay screen's content: current race + historic stats for
 * this circuit, the blue-shell button, the optional power-up grid, and a
 * "Race concluded?" button that navigates to a dedicated full-screen
 * position-entry screen (BattlePositionForm) rather than expanding
 * inline. The full-bleed backdrop and the live leaderboard live one
 * level up, in BattleScreen — shared with the track-picker screen so
 * they stay mounted (and the leaderboard stays visibly put) across the
 * switch between picking a track and racing.
 *
